<nav x-data="{ mobileMenuOpen: false }" class="fixed top-0 w-full z-50 bg-surface/95 backdrop-blur-md border-b border-outline-variant/30">
    <div class="flex justify-between items-center px-4 sm:px-6 md:px-margin-desktop py-3 md:py-4 max-w-container-max mx-auto">
        ...
        <!-- Logo + Brand -->
        <a class="flex items-center gap-2 hover:opacity-80 transition-opacity duration-300 min-w-0" href="{{ route('home') }}">
        ...
        <div class="hidden md:flex gap-5 lg:gap-8 items-center">
            <a class="font-label-lg text-label-lg whitespace-nowrap {{ Route::is('home') ? 'text-primary border-b-2 border-primary pb-1' : 'text-secondary hover:text-primary transition-colors' }}"
               href="{{ route('home') }}">{{ __('nav.home') }}</a>
            <a class="font-label-lg text-label-lg whitespace-nowrap {{ Route::is('about') ? 'text-primary border-b-2 border-primary pb-1' : 'text-secondary hover:text-primary transition-colors' }}"
               href="{{ route('about') }}">{{ __('nav.about') }}</a>
            <a class="font-label-lg text-label-lg whitespace-nowrap {{ Route::is('projects*') ? 'text-primary border-b-2 border-primary pb-1' : 'text-secondary hover:text-primary transition-colors' }}"
               href="{{ route('projects') }}">{{ __('nav.projects') }}</a>
            <a class="font-label-lg text-label-lg whitespace-nowrap {{ Route::is('contact') ? 'text-primary border-b-2 border-primary pb-1' : 'text-secondary hover:text-primary transition-colors' }}"
    ...
    <div x-show="mobileMenuOpen"
        ...
        <div class="flex flex-col px-4 sm:px-6 py-2">
            <a @click="mobileMenuOpen = false" href="{{ route('home') }}"
               class="py-4 border-b border-outline-variant/20 font-label-lg text-label-lg {{ Route::is('home') ? 'text-primary' : 'text-secondary' }}">
               {{ __('nav.home') }}
            </a>
            <a @click="mobileMenuOpen = false" href="{{ route('about') }}"
               class="py-4 border-b border-outline-variant/20 font-label-lg text-label-lg {{ Route::is('about') ? 'text-primary' : 'text-secondary' }}">
               {{ __('nav.about') }}
            </a>
            <a @click="mobileMenuOpen = false" href="{{ route('projects') }}"
               class="py-4 border-b border-outline-variant/20 font-label-lg text-label-lg {{ Route::is('projects*') ? 'text-primary' : 'text-secondary' }}">
               {{ __('nav.projects') }}
            </a>
            <a @click="mobileMenuOpen = false" href="{{ route('contact') }}"
               class="py-4 border-b border-outline-variant/20 font-label-lg text-label-lg {{ Route::is('contact') ? 'text-primary' : 'text-secondary' }}">
               {{ __('nav.contact') }}
            </a>
            <a @click="mobileMenuOpen = false" href="{{ route('contact') }}"
